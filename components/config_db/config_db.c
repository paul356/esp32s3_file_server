...
{
    size_t val_len = sizeof(*display_on);
    esp_err_t ret = nvs_read_blob(CONFIG_NAMESPACE, "display_on", display_on, &val_len);
    if (ret != ESP_OK && ret != ESP_ERR_NVS_NOT_FOUND)
    {
        return ret;
    }
    ...
    {
        esp_err_t ret2 = save_display_switch(true);
        if (ret2 != ESP_OK)
        {
            return ret2;
        }
...
{
    return nvs_write_blob(CONFIG_NAMESPACE, "display_on", &display_on, sizeof(display_on));
}
